...
- **Focus mode**: double-clicking any card (or, with the toolbar's "Focus Mode" toggle enabled, a single click) highlights that person's immediate family — parents, spouse, children, and siblings, each derived from the same `parent-child`/`spouse` edges used for collapse logic — with a glow/outline treatment, while every other node and connector dims. If a relative is currently hidden behind a collapsed sibling chip or branch toggle, focus mode auto-expands just enough to reveal it; everything else stays collapsed. Clicking elsewhere, pressing Escape, or an "Exit focus" control clears it.
